...
function solution(id_list, report, k) {
    ...
    eraseDuplicateReport.map((reportStr)=>{
        ...
        reportCountObj[reportedTarget] = reportCountObj[reportedTarget] ? reportCountObj[reportedTarget]+1 : 1
    })
    
    for (let name in reportCountObj) {
        const count = reportCountObj[name]
        if(count >= k)
            reportedTargetArr.push(name)
    }
    
    eraseDuplicateReport.map((reportStr)=>{
        const [reporter,reportedTarget] = reportStr.split(' ')
        if(reportedTargetArr.indexOf(reportedTarget) > -1){
            result[id_list.indexOf(reporter)] = result[id_list.indexOf(reporter)] ? result[id_list.indexOf(reporter)] + 1 : 1
        }
    })
...
const addCount = (target,idx)=>target[idx] = target[idx] ? target[idx]+1 : 1
